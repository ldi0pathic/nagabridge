...
Der Delta2-Adapter läuft aktuell im **Prototype Mode**. Er akzeptiert bereits MQTT/Bus-Commands und publiziert einen minimalen State-Stand.

## Topics

| Topic | Richtung | Beschreibung |
|---|---|---|
| `ecoflow/delta2/state` | Adapter → Bus | Aktueller Gerätezustand |
| `ecoflow/delta2/command` | Bus/MQTT → Adapter | Eingehende Steuer-Commands |

MQTT-seitig werden die Topics standardmäßig mit Prefix `nagabridge/` gespiegelt, z. B. `nagabridge/ecoflow/delta2/command`.
...
- Für Delta2 sind aktuell **keine numerischen Command-Parameter** implementiert.
...
| Feld | Typ | Bedeutung |
|---|---|---|
| `message_type` | `string` | Immer `delta2_status` |
...
## Intentional difference zu Delta2Max

- `message_type` ist absichtlich **`delta2_status`** (Delta2Max nutzt `delta2max_status`). **(intentional difference)**
- Der Topic-Pfad ist absichtlich gerätespezifisch (`ecoflow/delta2/...` statt `ecoflow/delta2max/...`). **(intentional difference)**
